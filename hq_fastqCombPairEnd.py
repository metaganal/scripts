...
separator = character that separates the name of the read from the part that
    ...
    Adopted from https://github.com/enormandeau/Scripts/blob/master/fastqCombinePairedEnd.py with modification
"""
...
try:
    in1 = sys.argv[1]
    in2 = sys.argv[2]
    title = str(in1.split(".1.humgenElim.fq")[0])
    #title = str(title1.split(".")[0])
    #print title
except:
    print __doc__
    sys.exit(1)
...
with myopen(title +".pairend" + outSuffix, "w") as out4:
    while not (s1_finished and s2_finished):
        ...
        if not s1_finished and s1.getShortname(separator) in seq2_dict:
            ...
            seq1_dict[s1.getShortname(separator)].write_to_file(out4)
            seq1_dict.pop(s1.getShortname(separator))

            seq2_dict[s1.getShortname(separator)].write_to_file(out4)
            seq2_dict.pop(s1.getShortname(separator))
                          

        if not s2_finished and s2.getShortname(separator) in seq1_dict:

            seq1_dict[s2.getShortname(separator)].write_to_file(out4)
            seq1_dict.pop(s2.getShortname(separator))
            seq2_dict[s2.getShortname(separator)].write_to_file(out4)
