...
{
	std::ifstream fasta;
	std::string fullgene, header, ecor1, hind3, bamh1, not1;
	std::vector<std::string> frags;
    ...
    if(argc<3)
    {
        std::cerr << "Usage :" << argv[0] << " <Path of input file> <Restriction enzyme to use>" << std::endl;
        std::cerr << "Restriction enzyme to use :" << std::endl;
	    std::cerr << "1. EcoRI" << std::endl;
	    std::cerr << "2. HindIII" << std::endl;
	    std::cerr << "3. BamHI" << std::endl;
	    std::cerr << "4. NotI" << std::endl;
	    std::cerr << "5. All of the above" << std::endl;
	}
	...
	switch(i)
	{
		...
		default:
		{
			std::cerr << "Invalid input, please enter a number between 1 - 5." << std::endl;
		}
	}
